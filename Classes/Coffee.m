...
@synthesize coffeeID, coffeeName, price, isDirty, isDetailViewHydrated, coffeeImage, title, content, tags;
...
+ (void) getInitialDataToDisplay:(NSString *)dbPath {
	...
	if (sqlite3_open([dbPath UTF8String], &database) == SQLITE_OK) {
		...
		const char *sql = "select coffeeID, coffeeName, tags from coffee";
		...
		if(sqlite3_prepare_v2(database, sql, -1, &selectstmt, NULL) == SQLITE_OK) {
			...
			while(sqlite3_step(selectstmt) == SQLITE_ROW) {
                ...
                coffeeObj.tags = [NSString stringWithUTF8String:(char *)sqlite3_column_text(selectstmt, 2)];
